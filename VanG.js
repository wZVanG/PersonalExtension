...
Plugins.add("bcp", /^https?\:\/\/bcpzonasegura\.viabcp\.com/gi, function(){
	/*var num_tarj = "4557880720307861", tarj = num_tarj.match(/[\d]{4}/gi), boxs = ["primero","segundo","tercero","cuarto"], i = 0;
	
	while(boxs[i]) $("#" + boxs[i]).val(tarj[i++]);*/
	
	/*var obj = $(".space-3:eq(1) td:eq(1)");

	obj.length == 1 && obj.html(obj.html().replace(/\d+,/,function(a){
		a = parseInt(a,10);
		return (a + 1) + ","
	}))*/
